...
export const LeftSide = ({ variant = 'desktop' }: Props) => {
    const { chat: currentChat, chats, setChat, setChats, setShowNewChat } = useChatStore()
    ...
    useEffect(() => {
        // se eu nao estiver filtrando nada e os chat estiverem atualizando por alguem motivo
        // atualizar a lista de chats atualizados
    ...
    useEffect(() => {
        const handleUpdateChat = (data: UpdateChatEvent) => {
            ...
            if (data.type === 'delete' && data.query.chat_id === currentChat?.id) {
                setChat(null)
                toast.info('Chat deleted with success', { position: 'top-center' })
            }
        }
    ...
    return (
        <div className={`bg-slate-100 dark:bg-slate-900 border-r border-slate-50 dark:border-slate-900 ${variant === 'mobile' ? 'w-auto' : 'w-96'} h-app overflow-auto`}>
            <NewChat />
                        ...
                        <div className="space-y-1 flex-1 truncate">
                            ...
                            {chat.last_message
                                ? <div className="flex items-center justify-between gap-4">
                                    <div className="text-sm font-semibold text-slate-800 dark:text-slate-300 truncate text-ellipsis">
                                        {chat.last_message.body ?
                                            ...
                                            : chat.last_message.attachment?.audio ?
                                                ...
                                                : chat.last_message.attachment?.file ? <div className="flex items-center gap-1">
                                    ...
                                    {chat.unseen_count > 0 ? <Badge>{chat.unseen_count}</Badge> : chat.last_message.from_user.id === user?.id &&
