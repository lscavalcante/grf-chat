...
const Chat = () => {
  ...
  const handleGetMessages = async () => {
    ...
    if (response.error || !response.data) {
      toast.error("Error to find messages", { position: "top-center" });
      return;
    }
      ...
      if (chatMessages && data.query.chat_id === chat?.id) {
        switch (data.type) {
          ...
          case "delete":
            setChatMessages(
              chatMessages.filter(
                (messsage) => messsage.id !== data.query.message_id
              )
            );
            break;
        }
        ...
        <div className="space-y-8 p-7" ref={bodyMessageRef}>
            {chatMessages?.map(data => (
                <div key={data.id} className={`flex ${data.from_user.id === user?.id ? "justify-end" : "justify-start"} `}>
                    <MessageItem data={data} onDelete={handleDeleteChatMessage}/>
                </div>
            ))}
        </div>
        }
