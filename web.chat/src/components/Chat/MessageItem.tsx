...
const FileMessage = ({ data }: { data: FileAttachment }) => (
  <div className="flex items-center">
    <a href={data.src} target="_blank">
      {data.content_type.startsWith("img") ? (
        <img
  ...
  return (
    <div className="flex gap-3 items-center">
      {data.from_user.id === user?.id && (
        <DropdownMenu>
          <DropdownMenuTrigger>
            <EllipsisVertical className="size-5 text-slate-500 dark:text-slate-300 hover:text-primary cursor-pointer -mt-5" />
          </DropdownMenuTrigger>
      ...
      <div className="space-y-1">
        <div
          className={`${
            data.from_user.id === user?.id
              ? "max-w-xs sm:max-w-md py-2 px-3 bg-primary rounded-l-md rounded-ee-md text-primary-foreground"
              : "py-2 px-3 bg-secondary rounded-r-md rounded-es-md text-slate-700 dark:text-slate-200 "
          }`}
        >
